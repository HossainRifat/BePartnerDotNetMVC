    ...
    public class InvestorController : Controller
    {
        ...
        public ActionResult Registration(InvestorPersonal In)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (In.In_Img != null)
                {
                    ...
                    string fileName = Path.GetFileName(In.In_Img.FileName);
                    string ex = Path.GetExtension(In.In_Img.FileName);
                    string fullpath = Path.Combine(path, Session["Email"].ToString()+ex);
                    In.In_Img.SaveAs(fullpath);  //save into img folder


                    // for data base
                    string databaseImgpath = fullpath;
                    Session["IMG_file"] = Session["Email"].ToString() + ex;
                }
        ...
        public ActionResult InLogin(InvestorLogin In, string ReturnUrl)
        {
            ...
            if(investor != null)
            {
                ...
                {
                    if (investor.Status.Equals("Invalid"))
                    {
                        ViewBag.Message = "Account is not valid yet.<br>Try again later.";
                    }
                    else
                    {
                        ...
                        return RedirectToAction("Index", "InHome");
                    }
