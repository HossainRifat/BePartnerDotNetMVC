...
function openchatForm() {
    document.getElementById("messaage").style.display = "none";
    if (document.getElementById("messaage").style.display == "block") {
        document.getElementById("messaage").style.display = "none";
    }
    else {
        document.getElementById("messaage").style.display = "block"
    }
...
function InvestorMvcSearch() {
    var n = document.getElementById("inputSearch").value;
    console.log(n);
    window.location.href = "/InHome/InBusinessDetails/"+n;
...
$("#inputSearch").keyup(function () {
    ...
    $.ajax({
        type: "GET",
        url: "/InHome/EN_Search/" + searchValue,
        success: function (rresponse) {
            var data = JSON.parse(rresponse);
            $.each(data, function (key, item) {
                var d = item.Company_Name;
                console.log(item.Company_Name);
                //return { label: item.Company_Name, value: item.Company_Name };
                $("#result").text(d);
                $("#result").attr("href", "/InHome/InBusinessDetails/" + d)
            })
